...
;;;
;;
;; Project derived CSV tokens into the reusable stream model.

;; project-csv-derived-token : (or/c delimited-derived-token? 'eof) csv-config? -> token-like?
;;   Convert a derived CSV token into a reusable-stream token-like value.

(provide project-csv-derived-token)
...
;; raw-eof->token : csv-config? -> token-like?
...
(define (raw-eof->token config)
  (wrap-token-with-pos 'eof
                       ...
                       (make-stream-position 1 1 0)
                       (csv-config-source-positions config)))

;; malformed-token->result : delimited-derived-token? csv-config? -> token-like?
;;   Project malformed input or raise in strict mode.
(define (malformed-token->result derived-token config)
  (case (csv-config-errors config)
    [(emit-unknown)
     (wrap-token-with-pos
    ...
    [(raise)
     ...
     (define end-pos
       (delimited-derived-token-end derived-token))
     (raise-read-error "unknown CSV input"
                       #f
                       ...
                       (- (position-offset end-pos)
                          (position-offset start-pos)))]
    [else
     (error 'project-csv-derived-token
            "unsupported CSV error policy: ~a"
            (csv-config-errors config))]))

;; visible-derived-token? : delimited-derived-token? csv-config? -> boolean?
...
;; plain-derived-token->result : delimited-derived-token? csv-config? -> token-like?
...
;; project-csv-derived-token : (or/c delimited-derived-token? 'eof) csv-config? -> token-like?
;;   Convert a derived CSV token into the reusable stream model.
(define (project-csv-derived-token derived-token config)
  (cond
