...
(module+ test
  ...
  (define recipe-ref-token
    (findf (lambda (token)
           ...
           sample-derived))
  (define paren-ref-token
    (findf (lambda (token)
             (makefile-derived-token-has-tag? token 'makefile-paren-variable-reference))
           sample-derived))
  ...
  (define special-target-token
    (findf (lambda (token)
           ...
           escaped-comment-derived))
  (define rule-delimiter-token
    (findf (lambda (token)
             (makefile-derived-token-has-tag? token 'makefile-rule-delimiter))
           sample-derived))
  (define recipe-separator-token
    (findf (lambda (token)
             (makefile-derived-token-has-tag? token 'makefile-recipe-separator))
           escaped-comment-derived))
  ...
  (check-not-false paren-ref-token)
  ...
  (check-not-false rule-delimiter-token)
  (check-not-false recipe-separator-token)
  ...
  (check-equal? (makefile-derived-token-text recipe-ref-token)
                "$(CC)")
  (check-equal? (makefile-derived-token-text rule-delimiter-token)
                ":")
  (check-equal? (makefile-derived-token-text recipe-separator-token)
                ";")
