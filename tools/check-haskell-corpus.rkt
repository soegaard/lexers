...
;;;
;;; Haskell Corpus Checker
;;;
;;
;; Round-trip and timeout checks for a local Haskell corpus.
...
(require parser-tools/lex
         ...
         racket/string
         rackunit
         "../lexers-lib/lexers/haskell.rkt")

(define corpus-root
  "/tmp/lexers-haskell-corpus")
...
(define (skip-if-missing!)
  (unless (directory-exists? corpus-root)
    (displayln "Skipping Haskell corpus check: local corpus is unavailable.")
    (exit 0)))
...
;;   Check source round-trips through both public Haskell APIs.
(define (summarize-roundtrip source)
  (define derived
    (haskell-string->derived-tokens source))
  (define projected
    (haskell-string->tokens source #:profile 'coloring #:source-positions #f))
  (list (string=? source (apply string-append (map haskell-derived-token-text derived)))
