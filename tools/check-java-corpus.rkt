...
;;;
;;
;; Round-trip and timeout checks for a local Java corpus.
...
(define (skip-if-missing!)
  (unless (directory-exists? corpus-root)
    (displayln "Skipping Java corpus check: local corpus is unavailable.")
    (exit 0)))
...
;;   Check source round-trips through both public Java APIs.
(define (summarize-roundtrip source)
  (define derived
    (java-string->derived-tokens source))
  (define projected
    (java-string->tokens source #:profile 'coloring #:source-positions #f))
  (list (string=? source (apply string-append (map java-derived-token-text derived)))
