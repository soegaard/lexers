...
;;;
;;
;; Round-trip and timeout checks for a local Rust corpus.
...
(require parser-tools/lex
         ...
         racket/string
         rackunit
         "../lexers-lib/lexers/rust.rkt")

(define corpus-root
  "/tmp/lexers-rust-corpus")
...
(define (skip-if-missing!)
  (unless (directory-exists? corpus-root)
    (displayln "Skipping Rust corpus check: local corpus is unavailable.")
    (exit 0)))
...
;;   Check source round-trips through both public Rust APIs.
(define (summarize-roundtrip source)
  (define derived
    (rust-string->derived-tokens source))
  (define projected
    (rust-string->tokens source #:profile 'coloring #:source-positions #f))
  (list (string=? source (apply string-append (map rust-derived-token-text derived)))
