...
;;;
;;
;; Development utility for running the JSON lexer across a copied JSON corpus in
...
(define summary-path
  "/tmp/lexers-json-corpus/summary.txt")
...
;;   Collect copied JSON files from the corpus directory.
(define (corpus-files)
  (sort
     ...
     (lambda ()
       (channel-put
        ...
        (with-handlers ([exn:fail? (lambda (exn) (list 'error (exn-message exn)))])
          ...
          (define derived-text
            (apply string-append
                   (map json-derived-token-text derived)))
...
;; main : -> void?
;;   Run the JSON lexer across the copied corpus.
(define (main)
  (cond
    ...
    [else
     (printf "Skipping JSON corpus check; missing corpus directory ~a\n"
             corpus-root)]))
