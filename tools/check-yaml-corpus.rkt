...
;;;
;;
;; Development utility for running the YAML lexer across a copied YAML corpus in
...
;;   Copied YAML corpus root.
(define corpus-root
  "/tmp/lexers-yaml-corpus")

;; summary-path : path-string?
;;   Output summary written after the corpus run.
(define summary-path
  "/tmp/lexers-yaml-corpus/summary.txt")
...
;;   Collect copied YAML files from the corpus directory.
(define (corpus-files)
  (sort
     ...
     (lambda ()
       (channel-put
        ...
        (with-handlers ([exn:fail? (lambda (exn) (list 'error (exn-message exn)))])
          ...
          (define derived-text
            (apply string-append
                   (map yaml-derived-token-text derived)))
...
;; main : -> void?
;;   Run the YAML lexer across the copied corpus.
(define (main)
  (cond
    ...
    [else
     (printf "Skipping YAML corpus check; missing corpus directory ~a\n"
             corpus-root)]))
