...
;;;
;;; Python Corpus Checker
;;;
;;
;; Development utility for running the Python lexer across a copied Python
...
(define summary-path
  "/tmp/lexers-python-corpus/summary.txt")
...
;;   Collect copied Python files from the corpus directory.
(define (corpus-files)
  (sort
     ...
     (lambda ()
       (channel-put
        ...
        (with-handlers ([exn:fail? (lambda (exn) (list 'error (exn-message exn)))])
          ...
          (define derived-text
            (apply string-append
                   (map python-derived-token-text derived)))
...
;; main : -> void?
;;   Run the Python lexer across the copied corpus.
(define (main)
  (cond
    ...
    [else
     (printf "Skipping Python corpus check; missing corpus directory ~a\n"
             corpus-root)]))
