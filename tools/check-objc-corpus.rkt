...
;;;
;;; Objective-C Corpus Checker
;;;
;;
;; Development utility for running the Objective-C lexer across a copied
;; Objective-C corpus in /tmp with per-file timeouts and round-trip checks.
...
;;   Copied Objective-C corpus root.
(define corpus-root
  "/tmp/lexers-objc-corpus")

;; summary-path : path-string?
;;   Output summary written after the corpus run.
(define summary-path
  "/tmp/lexers-objc-corpus/summary.txt")
...
;;   Collect copied Objective-C files from the corpus directory.
(define (corpus-files)
  (sort
     ...
     (lambda ()
       (channel-put
        ...
        (with-handlers ([exn:fail? (lambda (exn) (list 'error (exn-message exn)))])
          ...
          (define derived-text
            (apply string-append
                   (map objc-derived-token-text derived)))
...
;; main : -> void?
;;   Run the Objective-C lexer across the copied corpus.
(define (main)
  (cond
    ...
    [else
     (printf "Skipping Objective-C corpus check; missing corpus directory ~a\n"
             corpus-root)]))
