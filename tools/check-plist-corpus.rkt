...
;;;
;;; Plist Corpus Checker
;;;
;;
;; Development utility for running the plist lexer across a copied XML plist
...
(define corpus-root
  "/tmp/lexers-plist-corpus")

(define summary-path
  "/tmp/lexers-plist-corpus/summary.txt")
     ...
     (lambda ()
       (channel-put
        ...
        (with-handlers ([exn:fail? (lambda (exn) (list 'error (exn-message exn)))])
          ...
          (define derived-text
            (apply string-append
                   (map plist-derived-token-text derived)))
...
(define (main)
  (cond
    ...
    [else
     (printf "Skipping plist corpus check; missing corpus directory ~a\n"
             corpus-root)]))
