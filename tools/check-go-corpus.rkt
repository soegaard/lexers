...
;;;
;;
;; Round-trip and timeout checks for a local Go corpus.
...
(require parser-tools/lex
         ...
         racket/string
         rackunit
         "../lexers-lib/lexers/go.rkt")

(define corpus-root
  "/tmp/lexers-go-corpus")
...
(define (skip-if-missing!)
  (unless (directory-exists? corpus-root)
    (displayln "Skipping Go corpus check: local corpus is unavailable.")
    (exit 0)))
...
;;   Check source round-trips through both public Go APIs.
(define (summarize-roundtrip source)
  (define derived
    (go-string->derived-tokens source))
  (define projected
    (go-string->tokens source #:profile 'coloring #:source-positions #f))
  (list (string=? source (apply string-append (map go-derived-token-text derived)))
