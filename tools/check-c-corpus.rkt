...
(define summary-path
  "/tmp/lexers-c-corpus/summary.txt")
        ...
        (with-handlers ([exn:fail? (lambda (exn) (list 'error (exn-message exn)))])
          ...
          (define projected
            (c-string->tokens source
                              ...
                              #:source-positions #f))
          (define derived
            (c-string->derived-tokens source))
          (define derived-text
            (apply string-append
                   (map c-derived-token-text derived)))
...
(define (main)
  (cond
    ...
    [else
     (printf "Skipping C corpus check; missing corpus directory ~a\n"
             corpus-root)]))
