...
;;;
;;; Mathematica Corpus Checker
;;;
;;
;; Round-trip and timeout checks for a local Mathematica corpus.
...
(require parser-tools/lex
         ...
         racket/string
         rackunit
         "../lexers-lib/lexers/mathematica.rkt")
...
(define (skip-if-missing!)
  (unless (directory-exists? corpus-root)
    (displayln "Skipping Mathematica corpus check: local corpus is unavailable.")
    (exit 0)))
...
;; summarize-roundtrip : string? -> list?
;;   Check source round-trips through both public Mathematica APIs.
(define (summarize-roundtrip source)
  (define derived
    (mathematica-string->derived-tokens source))
  (define projected
    (mathematica-string->tokens source
                                #:profile 'coloring
                                #:source-positions #f))
  (list (string=? source (apply string-append (map mathematica-derived-token-text derived)))
