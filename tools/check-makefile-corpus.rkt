...
;;;
;;; Makefile Corpus Checker
;;;
;;
;; Development utility for running the Makefile lexer across a copied
;; Makefile corpus in /tmp with per-file timeouts and round-trip checks.
...
(define summary-path
  "/tmp/lexers-makefile-corpus/summary.txt")
...
;; corpus-file? : path? -> boolean?
;;   Determine whether one path is a copied Makefile corpus input.
(define (corpus-file? p)
  (define name
    (path->string (file-name-from-path p)))
  (define lower
    (string-downcase name))
  (or (string-suffix? lower "makefile")
      (string-suffix? lower "gnumakefile")
      (string-suffix? lower ".mk")))

;; corpus-files : -> (listof path?)
;;   Collect copied Makefile files from the corpus directory.
(define (corpus-files)
  (sort
     ...
     (lambda ()
       (channel-put
        ...
        (with-handlers ([exn:fail? (lambda (exn) (list 'error (exn-message exn)))])
          ...
          (define derived-text
            (apply string-append
                   (map makefile-derived-token-text derived)))
...
;; main : -> void?
;;   Run the Makefile lexer across the copied corpus.
(define (main)
  (cond
    ...
    [else
     (printf "Skipping Makefile corpus check; missing corpus directory ~a\n"
             corpus-root)]))
