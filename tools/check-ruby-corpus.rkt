...
;;;
;;
;; Round-trip and timeout checks for a local Ruby corpus.
...
(define (skip-if-missing!)
  (unless (directory-exists? corpus-root)
    (displayln "Skipping Ruby corpus check: local corpus is unavailable.")
    (exit 0)))
...
;;   Check source round-trips through both public Ruby APIs.
(define (summarize-roundtrip source)
  (define derived
    (ruby-string->derived-tokens source))
  (define projected
    (ruby-string->tokens source
                         #:profile 'coloring
                         #:source-positions #f))
  (list (string=? source (apply string-append (map ruby-derived-token-text derived)))
