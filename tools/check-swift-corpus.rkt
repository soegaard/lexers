...
;;;
;;; Swift Corpus Checker
;;;
;;
;; Development utility for running the Swift lexer across a copied Swift
...
;;   Copied Swift corpus root.
(define corpus-root
  "/tmp/lexers-swift-corpus")

;; summary-path : path-string?
;;   Output summary written after the corpus run.
(define summary-path
  "/tmp/lexers-swift-corpus/summary.txt")
...
;;   Collect copied Swift files from the corpus directory.
(define (corpus-files)
  (sort
     ...
     (lambda ()
       (channel-put
        ...
        (with-handlers ([exn:fail? (lambda (exn) (list 'error (exn-message exn)))])
          ...
          (define derived-text
            (apply string-append
                   (map swift-derived-token-text derived)))
...
;; main : -> void?
;;   Run the Swift lexer across the copied corpus.
(define (main)
  (cond
    ...
    [else
     (printf "Skipping Swift corpus check; missing corpus directory ~a\n"
             corpus-root)]))
